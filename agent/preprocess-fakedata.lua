...
function main()
	...
	cmd:option('-input', '', [[Path to Input data for vocab]])
	...
	print("Vocab size:", dict_to_idx:size())
	...
	X = torch.Tensor(num_lines, opt.num_feats)
	X[{{1,num_lines/2},{}}] = torch.ceil(torch.rand(num_lines/2, opt.num_feats)*10)

	X[{{num_lines/2+1, num_lines},{}}] = torch.ceil(torch.rand(num_lines/2, opt.num_feats)*10) + 10

	y = torch.Tensor(num_lines, 1)
	y[{{1,num_lines/2},{}}] = 1
	y[{{num_lines/2+1, num_lines},{}}] = 2
